                      ...
                      struct route *route)
{
    ...
    if ((rc = vrf_l3_route_action(vrf, OFPROTO_ROUTE_ADD, ofp_route)) == 0) {
        ...
        VLOG_DBG("Unable to add route for %s. rc %d", route->prefix, rc);
    }
                         ...
                         struct route *route, bool del_route)
{
    ...
    if ((rc = vrf_l3_route_action(vrf, action, ofp_route)) == 0) {
        ...
        VLOG_DBG("Unable to delete route for %s. rc %d", route->prefix, rc);
    }
    for (i = 0; i < ofp_route->n_nexthops; i++) {
        free(ofp_route->nexthops[i].id);
    }
...
vrf_reconfigure_routes(struct vrf *vrf)
{
    ...
    SHASH_FOR_EACH(shash_route_row, &current_idl_routes) {
        route_row = shash_route_row->data;
        VLOG_DBG("route in db '%s/%s'", route_row->from, route_row->prefix);
    }
    HMAP_FOR_EACH_SAFE(route, next, node, &vrf->all_routes) {
    ...
    /* HALON_TODO : for port deletion, delete all routes in ofproto that has
     * NH as the deleted port. */
    /* HALON_TODO : for VRF deletion, delete all routes in ofproto that has
     * NH as any of the ports in the deleted VRF */
}

/* HALON_TODO : move vrf functions from bridge.c to this file */
/* HALON_TODO : move neighbor functions from bridge.c to this file */
