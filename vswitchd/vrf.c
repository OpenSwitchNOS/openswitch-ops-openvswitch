...
*/
void
vrf_port_reconfig_ipaddr(struct ofproto *ofproto, struct port *port,
                         struct ofproto_bundle_settings *bundle_setting)
{
    const struct ovsrec_port *idl_port = port->cfg;

    if (ofproto->ofproto_class->l3_host_action == NULL) {
        VLOG_DBG("No ofproto support to update port ip's.");
        return;
    }
