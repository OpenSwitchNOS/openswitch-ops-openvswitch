...
qos_configure(struct ofproto *ofproto)
{
   // cos map
   int n_modified;
   const struct ovsrec_qos_cos_map_entry *ovsrec_qos_cos_map_entry;
   struct cos_map_settings cos_map_settings;

   // how many rows in the COS map are modified?
   cos_map_settings.n_entries = 0;
   OVSREC_QOS_COS_MAP_ENTRY_FOR_EACH(ovsrec_qos_cos_map_entry, idl) {
      if (OVSREC_IDL_IS_ROW_MODIFIED(ovsrec_qos_cos_map_entry, idl_seqno) ||
          OVSREC_IDL_IS_ROW_INSERTED(ovsrec_qos_cos_map_entry, idl_seqno)) {
         VLOG_DBG("%s: MODIFIED %s %d", __FUNCTION__,
                  ovsrec_qos_cos_map_entry->description,
                  (int)ovsrec_qos_cos_map_entry->code_point);
      ...
      OVSREC_QOS_COS_MAP_ENTRY_FOR_EACH(ovsrec_qos_cos_map_entry, idl) {
             ...
             OVSREC_IDL_IS_ROW_INSERTED(ovsrec_qos_cos_map_entry, idl_seqno))
         {
            cos_map_settings.entries[n_modified].color =
                              qos_get_color(ovsrec_qos_cos_map_entry->color);
            cos_map_settings.entries[n_modified].codepoint =
                              ovsrec_qos_cos_map_entry->code_point;
            cos_map_settings.entries[n_modified].local_priority =
                              ovsrec_qos_cos_map_entry->local_priority;
            n_modified++;
         }
      }
   ...
   // dscp map
   const struct ovsrec_qos_dscp_map_entry *ovsrec_qos_dscp_map_entry;
   struct dscp_map_settings dscp_map_settings;

   // how many rows in the DSCP map are modified?
   dscp_map_settings.n_entries = 0;
   OVSREC_QOS_DSCP_MAP_ENTRY_FOR_EACH(ovsrec_qos_dscp_map_entry, idl) {
      if (OVSREC_IDL_IS_ROW_MODIFIED(ovsrec_qos_dscp_map_entry, idl_seqno) ||
          OVSREC_IDL_IS_ROW_INSERTED(ovsrec_qos_dscp_map_entry, idl_seqno)) {
         dscp_map_settings.n_entries++;
         VLOG_DBG("%s: MODIFIED %s %ld", __FUNCTION__,
                  ovsrec_qos_dscp_map_entry->description,
                  ovsrec_qos_dscp_map_entry->code_point);
      }
   }
