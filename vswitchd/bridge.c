...
bridge_reconfigure(const struct ovsrec_open_vswitch *ovs_cfg)
{
    ...
    struct blk_params bridge_blk_params = {0};
#else
...
#ifdef OPS
    bridge_blk_params.ovs_cfg = ovs_cfg;
    ...
    /* Execute the reconfigure for block BLK_INIT_UPDATE_DATA */
    execute_reconfigure_block(&bridge_blk_params, BLK_INIT_UPDATE_DATA);
#endif
...
#ifdef OPS
        /* Execute the reconfigure for block BLK_BR_UPDATE_PORTS */
        bridge_blk_params.br  = br;
        bridge_blk_params.vrf = NULL;
        execute_reconfigure_block(&bridge_blk_params, BLK_BR_UPDATE_PORTS);
#endif
    }
#ifndef OPS_TEMP
    free(splinter_vlans);
    ...
    HMAP_FOR_EACH (vrf, node, &all_vrfs) {
        ...
        ** deleted ports */
        ...
        /* Execute the reconfigure for block BLK_VRF_UPDATE_PORTS */
         bridge_blk_params.br  = NULL;
         bridge_blk_params.vrf = vrf;
        execute_reconfigure_block(&bridge_blk_params, BLK_VRF_UPDATE_PORTS);
    }
#endif
...
#ifdef OPS
            /* Execute the reconfigure for block BLK_BR_UPDATE_PORTS_STEP2 */
            bridge_blk_params.br  = br;
            bridge_blk_params.vrf = NULL;
            execute_reconfigure_block(&bridge_blk_params, BLK_BR_UPDATE_PORTS_STEP2);
#endif
        }
    }
    ...
    HMAP_FOR_EACH (vrf, node, &all_vrfs) {
        if (vrf->up->ofproto) {
            ...
            /* Execute the reconfigure for block BLK_VRF_UPDATE_PORTS_STEP2 */
            bridge_blk_params.br  = NULL;
            bridge_blk_params.vrf = vrf;
            execute_reconfigure_block(&bridge_blk_params, BLK_VRF_UPDATE_PORTS_STEP2);
        }
    }
#endif
...
#ifdef OPS
        /* Execute the reconfigure for block BLK_BR_UPDATE_PORTS_STEP3 */
        bridge_blk_params.br  = br;
        bridge_blk_params.vrf = NULL;
        execute_reconfigure_block(&bridge_blk_params, BLK_BR_UPDATE_PORTS_STEP3);
#endif
        shash_destroy(&br->wanted_ports);
    }
    ...
    HMAP_FOR_EACH (vrf, node, &all_vrfs) {
        ...
        /* Execute the reconfigure for block BLK_VRF_UPDATE_PORTS_STEP3 */
        bridge_blk_params.br  = NULL;
        bridge_blk_params.vrf = vrf;
        execute_reconfigure_block(&bridge_blk_params, BLK_VRF_UPDATE_PORTS_STEP3);
...
#ifdef OPS
        /* Execute the reconfigure for block BLK_BR_CFG */
        bridge_blk_params.br  = br;
        bridge_blk_params.vrf = NULL;
        execute_reconfigure_block(&bridge_blk_params, BLK_BR_CFG);
    ...
    HMAP_FOR_EACH (vrf, node, &all_vrfs) {
        ...
        ** port_configure */
        if( is_port_configured ) {
            ...
            /* Execute the reconfigure for block BLK_CFG_NEIGHBORS */
            bridge_blk_params.br  = NULL;
            bridge_blk_params.vrf = vrf;
            execute_reconfigure_block(&bridge_blk_params, BLK_CFG_NEIGHBORS);
        }
        ...
        ** table. */
        ...
        /* Execute the reconfigure for block BLK_CFG_NEIGHBORS_STEP2 */
        bridge_blk_params.br  = NULL;
        bridge_blk_params.vrf = vrf;
        execute_reconfigure_block(&bridge_blk_params, BLK_CFG_NEIGHBORS_STEP2);
