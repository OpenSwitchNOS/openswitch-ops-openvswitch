...
static void vsctl_port_set_vlan_tag(struct ovsrec_bridge *bridge,
                                    struct ovsrec_port *port,
                                    int vlan)
{
    for (int i = 0; i < bridge->n_vlans; i++) {
        if (bridge->vlans[i]->id == vlan) {
            ovsrec_port_set_vlan_tag(port, bridge->vlans[i]);
            break;
        }
    }
}
...
cmd_add_br(struct ctl_context *ctx)
{
    ...
    if (!parent_name) {
        ...
        vsctl_port_set_vlan_tag(br, port, vlan);
         ...
         char *settings[], int n_settings)
{
    ...
    if (bridge->parent) {
        vsctl_port_set_vlan_tag(bridge->br_cfg, port, bridge->vlan);
    }
