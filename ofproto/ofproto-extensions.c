...
int register_ofproto_extension(int magic, void *ptr)
{
    ...
    struct ofproto_extension_header *header =
        (struct ofproto_extension_header *)ptr;
    ...
    if (magic == 0) {
        VLOG_ERR(
            "[register_ofproto_extension] Error cannot add extention with null magic\n");
        goto err_inval_param;
    }

    if (ptr == NULL) {
        VLOG_ERR(
            "[register_ofproto_extension] Error cannot add extention with null ptr\n");
        goto err_inval_param;
    }

    if (magic != header->magic) {
        VLOG_ERR(
            "[register_ofproto_extension] Error magic and structure magic do \
...
err_inval_param:
    VLOG_ERR(
        "[register_ofproto_extension] Error: there is already an extension with \
         the magic [0x%08x]\n",
         magic);
    return EINVAL;
