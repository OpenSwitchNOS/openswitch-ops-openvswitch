...
struct plugin_extension_interface {
    ...
    void *plugin_functions; /**< Start of exported plugin functions */
};
...
int register_ofproto_extension(const char *plugin_name, void *plugin_functions)
{
    ...
    struct ofproto_extension_header *header = (struct ofproto_extension_header *)plugin_functions;
    ...
    if (!ext) {
        ...
        ext->plugin_functions = plugin_functions;
...
int unregister_ofproto_extension(const char *plugin_name)
{
    ...
    if (ext) {
      if (!shash_find_and_delete(&sh_extensions, plugin_name)){
        VLOG_ERR("Unable to delete extension with plugin_name [%s].", plugin_name);
        goto error;
      }
        ...
        const char *plugin_name, int major, int minor, void **plugin_interface)
{
    ...
    if (ext) {
        ...
        *plugin_interface = ext->plugin_functions;
        return 0;
    } else {
...
int find_asic_extension(int major, int minor, void **plugin_interface)
{
    ...
    return find_ofproto_extension(asic_plugin_name, major, minor, plugin_interface);
