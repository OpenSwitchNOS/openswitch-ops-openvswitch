...
ovsdb_idl_set_identity(struct ovsdb_idl *idl, char* name)
{
    ...
    json_object_put_string(params, "name", name);
...
/* Returns the current priority assigned to the IDL session */
struct uuid *
ovsdb_idl_get_session_uuid(struct ovsdb_idl *idl)
{
    return &idl->session_uuid;
}
        ...
        if (msg->type == JSONRPC_NOTIFY
            ...
            case IDL_S_IDENTIFY:
                if (msg->result->type == JSON_ARRAY
                   ...
                   && msg->result->u.array.elems[0]->type == JSON_OBJECT) {
                    const struct json *asigned_priority;
                    asigned_priority = shash_find_data(
                                       msg->result->u.array.elems[0]->u.object,
                                       "priority");
                    if (!asigned_priority) {
                        VLOG_ERR("Identify response with invalid format");
                    }
                    idl->session_priority = json_integer(asigned_priority);
                    const struct json *asigned_uuid;
                    asigned_uuid = shash_find_data(
                                   msg->result->u.array.elems[0]->u.object,
                                   "uuid");
                    uuid_from_string(&idl->session_uuid,
                                     json_string(asigned_uuid));
                } else {
                    OVS_NOT_REACHED();
                }
                break;
