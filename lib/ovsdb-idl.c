    ...
    for(i = 0; i < idl->class->n_tables; i++){
        ...
        if (table->class == tc) {
            ...
            if(!shash_add_once(&table->indexes,
                      index_name,
                      index)){
                VLOG_ABORT("Can't repeat index name '%s' at table %s",
                           ...
                           )
{
    ...
    if(!index->table->need_table &&
       !((OVSDB_IDL_MONITOR | OVSDB_IDL_ALERT) &
        ...
        VLOG_ABORT("Can't add column '%s' at index '%s' in "
    ...
    if(custom_comparer || column->compare) {
        ...
        VLOG_ABORT("Column %s doesn't have default comparator, and"
    ...
    for(i = 0; i < idl->class->n_tables; i++){
        ...
        if (table->class == tc) {
            cursor->index = (struct ovsdb_idl_index *) shash_find(
                                    ...
                                    index_name)->data;
            if(!cursor->index) {
                VLOG_ABORT("Cursor initialization fails. "
                                 ...
                                 const void *conf)
{
     ...
     */
    return index->row_sync ? (a < b) - (a > b) : 0;
}
