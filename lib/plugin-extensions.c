...
int register_plugin_extension(struct plugin_extension_interface *plugin_interface)
{
    ...
    VLOG_INFO("Register plugin_name %s plugin_function %p.",
               plugin_interface->plugin_name, plugin_interface);
    extension = shash_find_data(&sh_extensions, plugin_interface->plugin_name);
    ...
    if (!extension) {
        ...
        extension->plugin_name = plugin_interface->plugin_name;
        extension->major = plugin_interface->major;
        extension->minor = plugin_interface->minor;
        extension->plugin_interface = plugin_interface;
        if(!shash_add_once(&sh_extensions, plugin_interface->plugin_name,
                           extension)){
            free(extension);
