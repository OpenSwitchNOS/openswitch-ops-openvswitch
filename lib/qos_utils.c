...
/*
 * Initialize a single DSCP map entry.
 */
static void
...
/*
 * Allocate & initialize a DSCP map table with default values.
 * TBD: fetch initial values from YAML file.
 */
struct ovsrec_qos_dscp_map_entry *
qos_create_default_dscp_map(void) {
...
/*
 * Free up previously allocated DSCP map table.
 */
void
qos_destroy_default_dscp_map(struct ovsrec_qos_dscp_map_entry *dscp_map) {
    if (dscp_map == NULL) {
        return;
    }
...
/*
 * Initialize a single COS map entry.
 */
static void
...
/*
 * Allocate & initialize a COS map table with default values.
 * TBD: fetch initial values from YAML file.
 */
struct ovsrec_qos_cos_map_entry *
qos_create_default_cos_map(void) {

    struct ovsrec_qos_cos_map_entry *cos_map = xmalloc(
            sizeof(struct ovsrec_qos_cos_map_entry) * QOS_COS_MAP_ENTRY_COUNT);

    set_cos_map_entry(&cos_map[0], 0, 1, "green", "Best Effort");
    set_cos_map_entry(&cos_map[1], 1, 0, "green", "Background");
    set_cos_map_entry(&cos_map[2], 2, 2, "green", "Excellent Effort");
    set_cos_map_entry(&cos_map[3], 3, 3, "green", "Critical Applications");
    set_cos_map_entry(&cos_map[4], 4, 4, "green", "Video");
    set_cos_map_entry(&cos_map[5], 5, 5, "green", "Voice");
    set_cos_map_entry(&cos_map[6], 6, 6, "green", "Internetwork Control");
    set_cos_map_entry(&cos_map[7], 7, 7, "green", "Network Control");

    return cos_map;
}

/*
 * Free up previously allocated COS map table.
 */
void
qos_destroy_default_cos_map(struct ovsrec_qos_cos_map_entry *cos_map) {
