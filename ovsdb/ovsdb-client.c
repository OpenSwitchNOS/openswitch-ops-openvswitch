            ...
            int argc OVS_UNUSED, char *argv[] OVS_UNUSED)
{
    ...
    if(reply->result->u.array.elems[0]->type != JSON_OBJECT) {
        ovs_fatal(0, "identify response isn't an object");
    }

    struct json *response = shash_find_data(
            reply->result->u.array.elems[0]->u.object, "priority");
    if (!response) {
