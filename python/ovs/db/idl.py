...
"""
    ...
    def __process_update(self, table, uuid, old, new):
        ...
        if not new:
            ...
            if row:
                ...
                if hasattr(self._db.tables[table.name], 'track_list'):
                    ...
                    if _deleted.uuid in self._db.tables[table.name].track_list:
                        self._db.tables[table.name].track_list[_deleted.uuid].delete_seqno = seqno
                    else:
                        self._db.tables[table.name].track_list[_deleted.uuid] = TrackInfo(delete_seqno=seqno)
                else:
                    del table.rows[uuid]
            ...
            if not row:
                ...
                if hasattr(self._db.tables[table.name], 'track_list'):
                    seqno = self.change_seqno + 1
                    self._db.tables[table.name].track_list[row.uuid] = TrackInfo(create_seqno=seqno)
    ...
    def __row_update(self, table, row, row_json):
        ...
        if tracked_columns:
            ...
            if row.uuid in table.track_list:
                table.track_list[row.uuid].columns = table.track_list[row.uuid].columns | set(tracked_columns)
                table.track_list[row.uuid].update_seqno = seqno
            else:
                table.track_list[row.uuid] = TrackInfo(tracked_columns, update_seqno=seqno)
    ...
    def track_add_column(self, table, column):
        """
        Enabe tracking for 'column' in 'table'
        """
        ...
        if column not in self._db.tables[table].columns:
            raise error.Error("column %s in table %s is not registered with idl", table, column)
    ...
    def track_add_all(self):
        """
        Enable tracking for all registered 'columns' for all registered 'tables'
        """
    ...
    def track_clear_all(self):
        ...
        """
        for v in self._db.tables.itervalues():
            if hasattr(v, 'track_list'):
                v.track_list = TrackList()
    ...
    def track_get(self, table):
        ...
        if not hasattr(self._db.tables[table], 'track_list'):
            raise error.Error("table %s is not being tracked for changes", table)
...
class TrackInfo(object):
    def __init__(self, columns=[], create_seqno=0, delete_seqno=0, update_seqno=0):
